...
typedef enum Map_Result {
	...
	MAP_EMPTY_ERROR 			/**< Map is empty error 	*/
} Map_Result;
...
void PrintMap(const HashMap* _map); /*for testing*/
 ...
 */
void HashMap_Destroy(HashMap** _map,DestroyFunction _keyDes, DestroyFunction _valDes);
 ...
 * @retval  MAP_UNINITIALIZED_ERROR
 * @retval  MAP_EMPTY_ERROR if map is empty
 * @warning key must be unique and destinct
 */
 ...
 * @retval number of key-value if succeeded ,0 otherwise  
 */
