...
typedef void (*actionFunction)(const void*,void*);
...
/** @brief Print elements in the list with given action function FOR DEBUG
 * Average time complexity O(n).
 *
 * @params _list : A previously created List ADT returned via ListCreate
 * @params _actF : pointer to action function
 * @params _context : pointer to context of action function
 */
void List_ForEach(List* listp, actionFunction _action, void* _context);

/** @brief Find a node in the list of a given element.
 * Average time complexity O(n).
 *
 * @params _list : A previously created List ADT returned via ListCreate
 * @params _cmp : pointer to comperator function
 *
 * @returns:
 * @retval pointer to  node if found;
 * @retval pointer to tail node if not found;
 */
const void* List_FindElement(const List* _list ,const void* _element ,compareFunction _cmpF);
